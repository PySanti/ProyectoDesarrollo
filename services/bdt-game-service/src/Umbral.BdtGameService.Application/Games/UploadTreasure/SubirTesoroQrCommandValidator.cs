...
public sealed class SubirTesoroQrCommandValidator : AbstractValidator<SubirTesoroQrCommand>
{
    public const long MaxImageSizeBytes = 5 * 1024 * 1024;

    private static readonly string[] AllowedContentTypes = ["image/jpeg", "image/png"];

    public SubirTesoroQrCommandValidator()
    {
        RuleFor(command => command.PartidaId).NotEmpty();
        RuleFor(command => command.EtapaId).NotEmpty();
